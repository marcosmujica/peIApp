...
export class AIService {
  ...
  async predictRubro(description: string, type: 'income' | 'expense', allowedRubros?: any[]): Promise<string | null> {
    ...
    try {
      ...
      const prompt = `Clasifica esta descripción de transacción financiera en UNA y SOLO UNA de las categorías de la lista de abajo.

ID de Categorías Válidos:
${rubroList}
...
Reglas:
- Devuelve ÚNICAMENTE el ID crudo (ej: 'comida') y NADA más.
- Sin puntuación, sin formato markdown, sin explicaciones.`;
  ...
  private processResponse(data: any, rubros: any[]): string | null {
    ...
    const found = rubros.find(r => r.id === cleanText);
