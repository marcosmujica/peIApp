...
/**
 * Normaliza una fecha para evitar el desfase de zona horaria (clÃ¡sico error de un dÃ­a menos).
...
const parseSafeDate = (dateSource: any): Date => {
  ...
  // Al pasarlo a local en zonas horarias negativas (AmÃ©rica), resta un dÃ­a.
  if (typeof dateSource === 'string' && dateSource.length <= 10) {
    ...
    // o simplemente aÃ±adir una hora segura al mediodÃ­a.
...
const BASE_PAYMENT_METHODS = [
  ...
  { id: 'debit', label: 'Tarjeta de DÃ©bito' },
  { id: 'credit', label: 'Tarjeta de CrÃ©dito' },
...
const DATE_QUICK_OPTIONS = [
  ...
  { id: '15days', label: 'Dentro de 15 dÃ­as' },
...
const CURRENCIES = [
  ...
  { code: 'PEN', name: 'Sol' },
  { code: 'VES', name: 'BolÃ­var' },
  { code: 'BOB', name: 'Boliviano' },
  { code: 'CRC', name: 'ColÃ³n Costarricense' },
  { code: 'CUP', name: 'Peso Cubano' },
  { code: 'DOP', name: 'Peso Dominicano' },
  { code: 'GTQ', name: 'Quetzal' },
  { code: 'HNL', name: 'Lempira' },
  { code: 'NIO', name: 'CÃ³rdoba' },
  { code: 'PAB', name: 'Balboa' },
  { code: 'PYG', name: 'GuaranÃ­' },
];
...
export const AddMovementScreen = () => {
  ...
  // Si no hay ticketId (nuevo) es dueÃ±o. Si hay ticketId, DEBE haber un ownerId cargado y coincidir.
  ...
  const paymentMethods = useMemo(() => {
    ...
    if (user?.country === 'UY' || user?.country === 'Uruguay') {
      ...
      list.push(
        ...
        { id: 'pagofacil', label: 'Pago FÃ¡cil' },
  ...
  const performShare = async (sid: string) => {
    try {
      ...
      const message = `${userName} te enviÃ³ un ticket de PeiApp.\n\nPodÃ©s ver los detalles, postergar la fecha o cargar un pago acÃ¡:\n${shareUrl}\n\nÂ¡MejorÃ¡ tus finanzas con PeiApp! www.peiapp.tech`;
  ...
  const handleProrateAmount = () => {
    ...
    if (!isNaN(count) && count > 1 && !isNaN(currentAmount) && currentAmount > 0) {
      ...
      Alert.alert('Error', 'Ingresa un monto y cantidad de cuotas vÃ¡lidos (mayor a 1).');
    }
  };

  const aiPulseAnim = useRef(new Animated.Value(1)).current;

  // Manejar animaciÃ³n de pulso para la predicciÃ³n de IA
  useEffect(() => {
    if (isPredicting) {
      Animated.loop(
        Animated.sequence([
      ...
      const loadTicket = async () => {
        ...
        if (t) {
          ...
          try {
            ...
            // Convertir mensajes de auditorÃ­a (que empiezan con ***) en logs sintÃ©ticos
            ...
            // Combinar y ordenar por fecha descendente (mÃ¡s nuevos arriba) o ascendente segÃºn la UI
            // La UI parece iterar logs.map, asÃ­ que mantendremos el orden cronolÃ³gico o inverso.
          ...
          // 2. Intentar cargar rubro por ID (nuevo formato de mnemÃ³nicos)
          if (t.rubro) {
  ...
  const triggerRubroPrediction = async (forceInEdit: boolean = false) => {
    // Si estamos en ediciÃ³n y la descripciÃ³n no ha cambiado desde la original, no predecimos
    if (!!ticketId && description === originalDescription && !forceInEdit) return;

    // Permitimos predecir tanto en alta como en ediciÃ³n si hay descripciÃ³n suficiente
    ...
    try {
      let allowedRubros = [];
      if (selectedWallet) {
        // Usar la misma lÃ³gica que getAvailableRubros() para filtrar
      ...
      // Si la IA no estÃ¡ segura o no encuentra uno consistente entre los permitidos, 1
      // el backend deberÃ­a devolver null y aquÃ­ no actualizamos nada.
      if (predictedId) { 
          setSelectedRubroId(predictedId);
      } else {
          // Opcional: Si queremos borrar la selecciÃ³n previa si la IA ya no estÃ¡ segura
          // setSelectedRubroId(null); 
      }

      // Predecir siempre el rubro contrario, sin importar si ya seleccionÃ³ contacto o no.
  ...
  const performCancellation = async (reason?: string) => {
    ...
    try {
      ...
      const updatedNote = reason?.trim() 
        ? (privateNote ? `${privateNote}\nMOTIVO CANCELACIÃ“N: ${reason}` : `MOTIVO CANCELACIÃ“N: ${reason}`)
        : privateNote;
      ...
      const errorMsg = e.response?.data?.message || 'No se pudo cancelar el ticket. Revisa tu conexiÃ³n.';
  ...
  const handleSave = async () => {
    ...
    if (attachmentUri && (attachmentUri.startsWith('file://') || attachmentUri.startsWith('content://') || attachmentUri.startsWith('blob:'))) {
      try {
        ...
        // Si fallÃ³ y era un blob de la ediciÃ³n anterior, lo quitamos para que no rompa el save
    ...
    try {
      if (ticketId) {
        ...
        try {
          ...
          // Inyectar en Chat si hubo cambios en descripciÃ³n o rubro
          try {
            const changedFields = [];
            if (description !== originalDescription) changedFields.push(`la descripciÃ³n a "${description}"`);
            if (selectedRubroId !== originalRubroId) {
                const newRubroLabel = selectedRubro?.label || 'Sin CategorÃ­a';
            ...
            if (changedFields.length > 0) {
              ...
              const auditMsg = `*** Ticket ACTUALIZADO - El ${todayStr}, ${userName} modificÃ³: ${changedFields.join(', ')}`;
        ...
        for (const recipient of recipients) {
          ...
          try {
            ...
            // Si ya se cobrÃ³/pagÃ³, subir comprobante y registrar pago
            if (isAlreadyPaid) {
              try {
                ...
                const msg = `*** PAGO REGISTRADO de $${Number(amount).toLocaleString('es-AR')}. MÃ©todo: ${methodLabel}. Estado Ticket: CUMPLIDO (Saldado al inicio)`;
      ...
      // Mostrar Toast Global con opciÃ³n de Ver
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      showToast('Imagen adjunta con Ã©xito (Imagen)');
    }
  };
  ...
  const handleTakePhoto = async () => {
    ...
    if (!result.canceled) {
      setAttachmentUri(result.assets[0].uri);
      showToast('Foto capturada con Ã©xito (CÃ¡mara)');
    }
  };
  ...
  const handlePickDocument = async () => {
    try {
      ...
      if (!result.canceled) {
        setAttachmentUri(result.assets[0].uri);
        showToast('Documento adjunto con Ã©xito (Archivo)');
      }
    } catch (err) {
  ...
  const handleShareAttachment = async (uri: string) => {
    ...
    try {
      ...
      if (!(await Sharing.isAvailableAsync())) {
        Alert.alert('Error', 'El uso compartido no estÃ¡ disponible en este dispositivo');
        return;
      }
  ...
  const applyDateChange = async (newDate: Date) => {
    ...
    try {
      ...
      try {
        ...
        await ticketsApi.addChatMessage(
          ticketId, 
          `*** Ticket ACTUALIZADO - El ${todayStr}, ${userName} cambiÃ³ el vencimiento del ${oldDateStr} al ${newDateStr}`
        );
      } catch (chatErr) {
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      try {
        ...
        const msg = `*** PAGO REGISTRADO de $${parsedAmount.toLocaleString('es-AR')}. MÃ©todo: ${methodLabel}. Estado Ticket: ${updated.status === 'completed' ? 'CUMPLIDO' : 'PENDIENTE'}`;
        await ticketsApi.addChatMessage(
          ticketId, 
          msg, 
          user?.displayName || user?.phoneNumber, 
    ...
    try {
      ...
      if (parsed.amount !== undefined || parsed.detail || parsed.payment_method || parsed.user_id || parsed.rubro) {
        ...
        if (parsed.currency && myWallet && parsed.currency !== myWallet.currency) {
          Alert.alert(
            "Moneda Distinta",
            `No es posible cargar la informaciÃ³n porque la moneda del QR (${parsed.currency}) es distinta a la de mi billetera seleccionada (${myWallet.currency}).`
          );
          return;
        }
        ...
        Alert.alert("CÃ³digo QR Escaneado", `Datos: ${data}`);
      }
    } catch (e) {
      Alert.alert("CÃ³digo QR Escaneado", `Datos: ${data}`);
    }
  ...
  // El estado isCustomDateActive ya estÃ¡ definido arriba
  ...
  const handleSelectQuickDate = (optionId: string) => {
    if (optionId === 'custom') {
      if (Platform.OS === 'android') {
        ...
        // Pero en Android el picker es un diÃ¡logo nativo separado.
      } else {
            ...
            <View style={{ flex: 1, paddingHorizontal: 24, paddingTop: 16 }}>
              ...
              <View style={styles.amountInputContainer}>
                       ...
                       <View style={{ flex: 1 }}>
                         <Text style={{ fontSize: 12, color: '#0369A1', fontFamily: FontFamily.medium }}>
                           {amount !== originalAmount 
                             ? `EstÃ¡s cambiando el importe (antes $${Number(originalAmount).toLocaleString('es-AR')}).`
                         ...
                         <Text style={{ fontSize: 11, color: '#0369A1', opacity: 0.8, marginTop: 2 }}>
                           Al guardar se registrarÃ¡ en el historial de auditorÃ­a.
                         </Text>
                       </View>
                     </View>
                   </View>
               ...
               {isEdit && initialDueDate && (
                 ...
                 const diffText = diffDays > 0 ? `retrasÃ³ ${diffDays} dÃ­as` : `adelantÃ³ ${Math.abs(diffDays)} dÃ­as`;
...
{/* Opposite User Info (if editing) */}
               {(() => {
                   ...
                   <View style={{ width: 1, height: 40, backgroundColor: '#f2f2f0', marginHorizontal: 16 }} />
                    ...
                    >
                       <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                         <Text style={{ fontSize: 13, color: '#878778' }}>{type === 'income' ? 'Origen' : 'CategorÃ­a'}</Text>
                         {isPredicting && (
                ...
                <View style={{ width: '100%', marginTop: 16 }}>
                    ...
                    <View style={{ borderBottomWidth: 1, borderBottomColor: '#f2f2f0' }}>
                      ...
                      {(assignedContacts.length > 0) && (
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 12 }}>
                          ...
                          <TouchableOpacity onPress={handleClearSelection}>
                            <Text style={{ fontSize: 12, color: '#c05050', fontFamily: FontFamily.medium }}>Limpiar selecciÃ³n</Text>
                          </TouchableOpacity>
                        </View>
                      )}
                      ...
                      <View style={{ paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f2f2f0' }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                          <View style={{ flex: 1 }}>
                            <Text style={{ fontSize: 15, color: '#363630', fontFamily: FontFamily.semibold }}>Â¿Ya se {type === 'income' ? 'cobrÃ³' : 'pagÃ³'}?</Text>
                            <Text style={{ fontSize: 12, color: '#878778' }}>Marca esto si la transacciÃ³n ya se completÃ³</Text>
                          </View>
                          <Switch 
                            value={isAlreadyPaid}
                              ...
                              {createPayAttachment ? (
                                ...
                                <View style={{ flexDirection: 'row', gap: 8 }}>
                                  ...
                                  >
                                    ...
                                    <Text style={{ fontSize: 13, color: '#363630', fontFamily: FontFamily.medium }}>GalerÃ­a</Text>
                                  </TouchableOpacity>
                                  <TouchableOpacity 
                                    style={{ flex: 1, backgroundColor: '#f2f2f0', borderRadius: 10, paddingVertical: 10, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 6 }}
                                    onPress={async () => {
                                  ...
                                  >
                                    ...
                                    <Text style={{ fontSize: 13, color: '#363630', fontFamily: FontFamily.medium }}>CÃ¡mara</Text>
                                  </TouchableOpacity>
                                </View>
                              )}
                            </View>
                      ...
                      <View style={{ width: '100%', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f2f2f0' }}>
                          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            <View style={{ flex: 1 }}>
                              ...
                              <Text style={{ fontSize: 12, color: '#878778' }}>Repetir este ticket periÃ³dicamente o ingresar un pago en cuotas.</Text>
                            </View>
                            <Switch
                              value={isRecurring}
                                ...
                                <View style={{ flex: 2 }}>
                                  ...
                                  >
                                    <Text style={{ fontSize: 14, color: '#363630' }}>
                                      ...
                                      recurrenceFrequency === 'biweekly' ? '1 vez cada 15 dÃ­as' :
                                      ...
                                      recurrenceFrequency === 'yearly' ? '1 vez por aÃ±o' : 'Seleccionar'}
                                    </Text>
                  ...
                  <TextInput
                    style={{ fontSize: 15, color: '#363630', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f2f2f0', minHeight: 60 }}
                    placeholder="Nota privada (solo tÃº la verÃ¡s)"
                    placeholderTextColor="#878778"
                    value={privateNote}
                  ...
                  {!!ticketId && !isTransferOrAdjustment && (
                    <View style={{ marginTop: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f2f2f0' }}>
                      <Text style={{ fontSize: 13, color: '#878778', marginBottom: 8, fontFamily: FontFamily.bold }}>
                        {isOwner ? 'Tu calificaciÃ³n como DueÃ±o' : 'Tu calificaciÃ³n como Destinatario'}
                      </Text>
                      <View style={{ flexDirection: 'row', gap: 8 }}>
                        {[1, 2, 3, 4, 5].map((star) => (
                          <TouchableOpacity 
                      ...
                      {isOwner ? (
                        participantRating > 0 && (
                          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
                            <Text style={{ fontSize: 12, color: '#878778' }}>CalificaciÃ³n del destinatario: </Text>
                        ...
                        ownerRating > 0 && (
                          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
                            <Text style={{ fontSize: 12, color: '#878778' }}>CalificaciÃ³n del dueÃ±o: </Text>
                            <View style={{ flexDirection: 'row', gap: 2 }}>
                              {[1, 2, 3, 4, 5].map((s) => (
              ...
              {!attachmentUri ? (
                <View style={{ gap: 16 }}>
                  ...
                  >
                    ...
                    <Text style={{ fontSize: 16, color: '#363630' }}>GalerÃ­a de imÃ¡genes</Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
                    style={{ backgroundColor: '#f2f2f0', borderRadius: 16, padding: 20, flexDirection: 'row', alignItems: 'center', gap: 16, opacity: status === 'cancelled' ? 0.5 : 1 }} 
                  ...
                  >
                    ...
                    <Text style={{ fontSize: 16, color: '#363630' }}>CÃ¡mara</Text>
                  </TouchableOpacity>
                  <TouchableOpacity 
                    style={{ backgroundColor: '#f2f2f0', borderRadius: 16, padding: 20, flexDirection: 'row', alignItems: 'center', gap: 16, opacity: status === 'cancelled' ? 0.5 : 1 }} 
        ...
        <TouchableWithoutFeedback onPress={() => setIsLogModalVisible(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
            ...
            <FlatList
                     ...
                     <Text style={{ fontSize: 16, fontFamily: FontFamily.semibold, color: '#363630' }}>
                      ...
                      <Text style={{ fontSize: 13, color: '#878778', marginTop: 2 }}>
                        {item.action === 'audit_message' ? item.newValue.replace(/^\*\*\*\s+/, '') : 
                         ...
                         (item.newValue || item.comment || 'Sin descripciÃ³n')}
                      </Text>
              ...
              ListEmptyComponent={<Text style={{ textAlign: 'center', color: '#878778', marginTop: 40 }}>No hay movimientos aÃºn.</Text>}
            />
            </View>
          </TouchableWithoutFeedback>
        </View>
      ...
      <Modal visible={isRubroModalVisible} transparent animationType="slide" onRequestClose={() => setRubroModalVisible(false)}>
        <TouchableWithoutFeedback onPress={() => setRubroModalVisible(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <Text style={{ fontSize: 20, fontFamily: FontFamily.bold, color: '#363630' }}>{type === 'income' ? 'Origen / Rubro' : 'CategorÃ­a / Rubro'}</Text>
                ...
                if (item.isSeparator) {
                  return (
                    <View style={{ paddingVertical: 16, alignItems: 'center' }}>
                      ...
                      <Text style={{ fontSize: 11, color: '#b7b7ae', marginTop: 8, fontFamily: FontFamily.bold, textTransform: 'uppercase', letterSpacing: 1 }}>Resto de categorÃ­as</Text>
                    </View>
                  );
                }
                return (
                  ...
                  {payAttachment ? (
                    ...
                    <View style={{ flexDirection: 'row', gap: 12 }}>
                       ...
                       >
                         ...
                         <Text style={{ fontSize: 14, color: '#363630', fontFamily: FontFamily.medium }}>CÃ¡mara</Text>
                       </TouchableOpacity>
                    </View>
                  )}
                </View>
            ...
            <TouchableWithoutFeedback>
              <View style={{ backgroundColor: '#fff', borderRadius: 24, padding: 24, width: '85%', alignItems: 'center' }}>
                <Text style={{ fontSize: 18, fontFamily: FontFamily.bold, color: '#363630', marginBottom: 20, textAlign: 'center' }}>
                  {isCustomDateActive ? 'Seleccionar Fecha' : 'Â¿CuÃ¡ndo vence?'}
                </Text>
      ...
      <Modal visible={isCancelModalVisible} transparent animationType="fade" onRequestClose={() => setCancelModalVisible(false)}>
        <TouchableWithoutFeedback onPress={() => setCancelModalVisible(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 14, color: '#878778', marginBottom: 20, textAlign: 'center' }}>Explica brevemente el motivo de la cancelaciÃ³n.</Text>
            <TextInput 
          ...
          <View style={styles.modalOverlay}>
            <TouchableWithoutFeedback>
              <View style={[styles.modalContent, { height: 'auto', paddingBottom: 40 }]}>
                <View style={styles.modalHeader}>
                  <Text style={styles.modalTitle}>Frecuencia de RepeticiÃ³n</Text>
                ...
                <View style={{ padding: 12 }}>
                  {[
                    ...
                    { id: 'yearly', label: '1 vez por aÃ±o' },
                  ].map((freq) => (
                    <TouchableOpacity 
                      key={freq.id}
                      style={{ 
